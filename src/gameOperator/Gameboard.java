...
public class Gameboard {
	...
	private void setHighScore()
	{
		...
		try
		{
	    	...
	    	writer.newLine();
	    	if(elapsedMS <= fastestMS && success )
	    	{
	...
	public void render(Graphics2D g)
	{
		...
		g.drawString("Best:" + highScore, Game.WIDTH - DrawUtils.wordsWidth("Lead Score:" + highScore, scoreFont, g) - 20, 40 );
		g.setColor(Color.red);
		g.drawString("Time:" + formattedTime, 30, 90);
		g.setColor(Color.green);
		g.drawString("Fastest: "+ formatTime(fastestMS), Game.WIDTH - DrawUtils.wordsWidth("Best Time: " + formatTime(fastestMS), scoreFont, g)- 20, 90);
	}
	...
	private void resetPosition(Tile current, int row, int col)
	{
		...
		if(Math.abs(distX) < Tile.MOVES)
		{
			current.setX(current.getX() -distX);
		}
		
		if(Math.abs(distY) < Tile.MOVES)
		{
		...
		if(distX < 0)
		{
			current.setX(current.getX() +Tile.MOVES);
			
		}
		
		if (distY < 0)
		{
			current.setY(current.getY() + Tile.MOVES);
		}
		if(distX > 0)
		{
			current.setX(current.getX() - Tile.MOVES);
